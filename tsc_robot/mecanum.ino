...
void calculateSpeed(motors *toRet, uint16_t x, uint16_t y, uint16_t yaw)
{
    ...
    toRet->leftFront = (uint8_t)map(x + y + yaw, -5433, 5433, 0, 255);
    toRet->rightFront = (uint8_t)map(-x + y - yaw, -5433, 5433, 0, 255);
    toRet->rightRear = (uint8_t)map(-x + y + yaw, -5433, 5433, 0, 255);
    toRet->leftRear = (uint8_t)map(x + y - yaw, -5433, 5433, 0, 255);
